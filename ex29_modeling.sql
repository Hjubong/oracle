...
/*
	...
	2. 갱신 이상, Update Anomaly
		- 동일한 데이터가 2개 이상의 테이블에 동시 저장 > 그 중 1개는 수정했지만, 다른 1개를 수정못하면 둘 중 어느 것이 올바른 데이터인지 판단하지 못하는 상황
	
	3. 삭제 이상, Deletion Anomaly
		- 특정 테이블에서 데이터를 삭제할 때 원하지 않는 데이터까지 같이 삭제하는 상황
	
	함수 종속, Functionl Dependency
	- 하나의 테이블내에서 컬럼끼리의 관계 표현
	- 정규화를 '부분 함수 종속'이나 '이행 함수 종속'을 모두 없애고, 모든 컬럼의 관계를 '완전 함수 종속'으로 만드는 작업이다.
	...
	제 1 정규화, 1NF
	- 모든 컬럼(속성)은 원자값을 가진다.
	- 여러개의 분리 가능한 값을 1개 컬럼안에 넣지 말것
	- 1개 테이블 > (정규화) > 2개 이상의 테이블
	
	제 2 정규화, 2NF
	- 기본 키가 아닌 모든 컬럼은 키본키에 완전 함수 종속이 어야 한다.
