...
FROM TBLINSA
	WHERE JIKWI  IN ('부장') AND CITY IN ('서울');
...
SELECT *
FROM TBLINSA 
	WHERE SUDANG + BASICPAY >= 1500000 AND CITY IN ('서울');
...
SELECT *
FROM TBLINSA 
	WHERE BASICPAY * 12 >= 20000000 AND JIKWI IN ('과장','부장') AND CITY IN ('서울');
...
SELECT *
FROM TBLINSA 
	WHERE SSN LIKE '%07__-%' or SSN LIKE '%08__-%' OR SSN LIKE '%09__-%';
...
--요구사항.026.tblInsa
--서울/인천/경기에 살고 입사일이 1998년~2000년 사이인 직원들을 가져오시오.
SELECT *
FROM TBLINSA 
