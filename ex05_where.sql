...
SELECT * FROM TBLINSA 
	WHERE 1 = 1; --조건절에 반드시 컬럼이 포함되지 않아도 된다. 단순히 모든 데이터를 검색하기 위한 방법
...
-- between, in > 종종 회사에서 사용 금지!! > 성능 문제 > 비교 연산자보다 느림
...
SELECT * FROM TBLINSA WHERE NAME LIKE '_종_';
...
SELECT * FROM TBLCOUNTRY WHERE POPULATION is NOT null; --*** 이 방법을 더 많이 사용함
